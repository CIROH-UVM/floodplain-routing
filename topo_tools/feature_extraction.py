...
def extract_features(run_path, plot=False, subset=None):
    ...
    for reach in list(valid_reaches):
        ...
        bkf_w = 13.0 * (tmp_meta['TotDASqKm'] ** 0.448)  # Underwood et al. 2021 VT regression  # 3.12 * (tmp_meta['TotDASqKm'] ** 0.415)  # Bieger et al. 2015 App. Highlands regression
        ...
        if edz_count == 0:
            ...
            features.loc[reach, 'w_edap_scaled'] = main_edz['w_edap'] / (13.0 * (tmp_meta['TotDASqKm'] ** 0.448))  # (3.12 * (tmp_meta['TotDASqKm'] ** 0.415))
            features.loc[reach, 'w_edep_scaled'] = main_edz['w_edep'] / (13.0 * (tmp_meta['TotDASqKm'] ** 0.448))  # (3.12 * (tmp_meta['TotDASqKm'] ** 0.415))
